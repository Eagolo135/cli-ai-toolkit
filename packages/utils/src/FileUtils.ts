...
export class FileUtils {
     ...
     */
    static async saveReference(content: string, slugPrompt: string, outputDir?: string): Promise<string> {
        const directory = outputDir || 'references';
        return this.saveFile(content, slugPrompt, directory, 'web_search', '.txt');
    }

    /**
     * Save AI feedback with robust error handling
     */
    static async saveAIFeedback(content: string, slugPrompt: string, outputDir?: string): Promise<string> {
        const directory = outputDir || path.join('references', 'aI_feedback');
        return this.saveFile(content, slugPrompt, directory, 'gemini', '.txt');
    }

    /**
     * Save image with robust error handling
     */
    static async saveImage(imageBuffer: Buffer, slugPrompt: string, outputDir?: string): Promise<string> {
        const directory = outputDir || 'images';
        return this.saveFile(imageBuffer, slugPrompt, directory, 'image', '.png');
    }

    /**
     * Save screenshot with robust error handling
     */
    static async saveScreenshot(imageBuffer: Buffer, slugPrompt: string, outputDir?: string): Promise<string> {
        const directory = outputDir || path.join('images', 'screenshots');
        return this.saveFile(imageBuffer, slugPrompt, directory, 'screenshot', '.png');
    }
    ...
    /**
     * Generate timestamp for filename (ISO format with dashes)
     */
    ...
    /**
     * Generate filename with timestamp__tool__slug pattern
     */
    static generateFilename(toolName: string, slug: string, extension: string): string {
        const timestamp = this.generateTimestamp();
        const sanitizedSlug = this.sanitizeSlug(slug);
        return `${timestamp}__${toolName}__${sanitizedSlug}${extension}`;
    }
