...
import { ScreenshotService, ScreenshotOptions } from '../services/ScreenshotService.js';
...
export class ScreenshotCommand implements Command {
    ...
    async execute(options: any): Promise<void> {
        try {
            ...
            const full = viewportOnly ? false : (options.options?.full !== undefined ? options.options.full : true); // Default to full page unless viewport-only
            ...
            // Capture screenshot
            const result = await this.screenshotService.captureScreenshot(screenshotOptions);
